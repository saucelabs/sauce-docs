module.exports = function (context) {
  ...
  if (!themeConfig.beamer) {
    throw new Error(
      'You need to specify `beamer` object in `themeConfig` ' + 'with `product_id` field in it to use beamer'
    );
  }
  if (!themeConfig.beamer.product_id) {
    throw new Error(
      'You specified the `goatCounter` object in `themeConfig`, ' + 'but the `product_id` field was missing. '
    );
  }
