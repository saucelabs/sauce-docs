// Based on https://github.com/remarkjs/remark-unwrap-images/blob/main/lib/index.js
// Used to prevent insertion of paragraph tags for all JSX children. Keeps behavior of MDX1 while
// allowing us to adopt newer versions until we can refactor everything.
/**
    ...
    return function (tree) {
        visit(tree, 'paragraph', function (node, index, parent) {
            if (
                parent &&
                typeof index === 'number' &&
                applicable(node, parent, false) === containsImage
            ) {
 ...
 *   Info.
 */
function applicable(node, parent, inLink) {
    /** @type {1 | 2 | 3} */
    let image = unknown;
    let index = -1;
    ...
    while (++index < node.children.length) {
        ...
        if (parent.type === 'mdxJsxFlowElement') {
            image = containsImage;
        } else if (child.type === 'text' && whitespace(child.value)) {
            // Whitespace is fine.
