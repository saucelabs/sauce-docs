...
For example, adding `127.0.0.1   mockserver` to your `/etc/hosts` file, then starting a server on `localhost:3333` would route mockserver:3333 HTTP calls to your local server. Mobile tests using Sauce Connect would be able to find your local server regardless of the nature of your test. 
