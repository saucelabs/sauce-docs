...
* On Linux, Sauce Connect Proxy looks for these variables, in this order:
  ...
  * 'HTTP_PROXY'
  * 'all_proxy'
  * 'ALL_PROXY' (they can be in the form 'ht<span>tp://</span>host.name:port' or 'host.name:port')
...
| '--pac <url>' | Proxy auto-configuration (can be a http(s) or local file: //URL). Absolute paths are required when specifying a local PAC file (e.g., fi<span>le:///</span>Users/Andrew/Desktop/MyPac.pac).
...
'''$ sc_download/bin/sc -u $SAUCE_USERNAME -k $SAUCE_ACCESS_KEY -p $PROXY_HOST:$PROXY_PORT  -w $PROXY_USERNAME:$PROXY_PASSWORD -T;o.......'''
...
To have Sauce Connect Proxy with your PAC file, use the '--pac <url>' command line option when starting Sauce Connect Proxy. The url argument can be an http(s) or local file: //URL. Absolute paths are required when specifying a local PAC file (e.g., fi<span>le:///</span>Users/Andrew/Desktop/MyPac.pac).
...
![Site Under Test (SUT) behind a proxy](static/img/sauce-connect/scp-sut-proxy.png)
...
![Network traffic flow With tunnel established using '-p'](static/img/sauce-connect/scp-traffic-flow-tunnel.png)
...
![Network traffic flow through a tunnel started with '-p'](static/img/sauce-connect/scp-traffic-flow-through-tunnel.png)
...
![Network traffic flow through a tunnel established using '-T'](static/img/sauce-connect/scp-traffic-flow-t.png)
...
![Network traffic flow through a tunnel started using '-T'](static/img/sauce-connect/scp-traffic-flow-through-t.png)
...
![Charles SSL certificates navigation](static/img/sauce-connect/charles-ssl-cert-nav.png)
...
![Charles Proxy settings navigation](static/img/sauce-connect/charles-proxy-settings.png)
