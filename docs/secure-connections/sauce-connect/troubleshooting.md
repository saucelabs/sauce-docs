...
Is there a firewall or proxy server in place between your machine running Sauce Connect and Sauce Labs ('*.saucelabs.com:443')? You may need to allow access in your firewall rules or configure Sauce Connect to use an additional proxy. See [Setup with Additional Proxies](/secure-connections/setup-configuration/additional-proxies)
...
As a primer, the diagram below is the ideal network configuration with regards to Sauce Connect Proxy. Your firewall only needs to allow for outbound connections to Sauce Labs. Sauce Connect establishes a TLS connection (tunnel) to a dedicated tunnel endpoint server hosted in the Sauce Labs cloud. For best performance, Sauce Connect should be running close to the Site Under Test or Application Under Test.

What Not to Do diagram - correct config.png

The diagrams below illustrate common configuration mistakes that result in dysfunctional setups. Make sure to avoid these when setting up Sauce Connect!
...
Dysfunctional SC setup Copy-2.png
