...
For more information, see [DMZ (computing)](https://en.wikipedia.org/wiki/DMZ_(computing)) and [What Not to Do: Common Mistakes in Sauce Connect Proxy Network Configurations](https://wiki.saucelabs.com/display/DOCS/What+Not+to+Do%3A+Common+Mistakes+in+Sauce+Connect+Proxy+Network+Configurations).
...
#### OS X
On OS X machines, certificates are pre-installed as part of the [Trust Store](https://support.apple.com/en-us/HT204132) and are accessible through the [Keychain](https://en.wikipedia.org/wiki/Keychain_(software)). If Sauce Connect is installed on an OS X machine, no troubleshooting should be necessary as long as it can access the Keychain.
...
2. When needed, the Sauce Labs browser requests resources from the Sauce Connect server.
3. Sauce Connect server passes that request to the Sauce Connect client, running on your side. All SSL internet traffic between the Sauce Connect client (on your network) and the Sauce Connect server (inside our network) is encrypted twice: once by the original server and again by Sauce Connect.
4. Sauce Connect client fetches the resource and returns it through the encrypted connection, back to the Sauce Connect server.
5. Sauce Connect server decrypts the traffic. If it's SSL traffic, the Sauce Connect server decrypts it again.
6. Sauce Connect re-encrypts SSL traffic using the CyberVillains certificate and returns it to the browser. Re-encryption only occurs once the traffic is safely received by the Sauce Labs network. SSL Bumping impacts only the traffic being returned to the browser through Sauce Connect.
