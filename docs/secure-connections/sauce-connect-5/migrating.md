...
**NOTE**: `tls-passthrough-domains` and `tls-resign-domains` flags cannot be used at the same time. Also, `direct-domains` and `tunnel-domains` cannot be used at the same time.
