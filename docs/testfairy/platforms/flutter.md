---
id: flutter
title: Adding Sauce Labs Mobile App Distribution to Your Flutter Project
...
If you are developing your app in Flutter, follow the instructions in this document add the Sauce Labs Mobile App Distribution SDK.
...
To use Sauce Labs Mobile App Distribution with the latest **stable** Flutter channel, you must set the minimum version for the plugin as 2.1.0.

To use Sauce Labs Mobile App Distribution with the latest **unstable** Flutter channel, you must clone this repository and use it as an offline dependency instead of the published version in pub.
...
- **I see `Specs satisfying the Sauce Labs Mobile App Distribution dependency were found, but they required a higher minimum deployment target.` when I build an iOS app.**
...
- **I have my own `HttpOverrides.global` setup. How can I make it work with Sauce Labs Mobile App Distribution?**
...
- **I see `Automatically assigning platform ios with version 8.0` when I build.**
  ...
  Sauce Labs Mobile App Distribution supports iOS 9.0 and above. Change the build target accordingly in your Xcode project.

- **I see `Looks like Sauce Labs Mobile App Distribution has an upgrade to do... 1.X.Y+hotfixZ is the latest stable branch` or errors related to Jetifier in the logs when I call an SDK method.**
...
- **CocoaPods could not find compatible versions for pod "Sauce Labs Mobile App Distribution".**
  ...
  If some of the libraries you use need to be at specific versions, copy the necessary lines from your backed-up _Podfile.lock_ into the newly created one. Keep the lines related to Sauce Labs Mobile App Distribution (note the title case in the name) untouched.
