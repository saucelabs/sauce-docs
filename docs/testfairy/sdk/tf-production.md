---
id: tf-production
title: Sauce Labs Mobile App Distribution (TestFairy) in Production
sidebar_label: Sauce Labs Mobile App Distribution in Production
---
...
Running Sauce Labs Mobile App Distribution in production offers numerous benefits, such as gaining valuable insights into user behavior, detecting and resolving issues promptly, and continuously improving your app's performance. With Sauce Labs Mobile App Distribution, you can proactively monitor your production environment, gather valuable data, and make informed decisions to deliver a superior app to your users.

## Running Sauce Labs Mobile App Distribution in Production

When running Sauce Labs Mobile App Distribution in production, you may record sensitive data such as medical information, financial data or photos.
...
1. When recording sensitive data you **must** use Sauce Labs Mobile App Distribution's end-to-end encryption with your own private keys, so that only your team can see your sessions.
...
1. In case you are using Sauce Labs Mobile App Distribution for customer support to better understand your users in case of a technical issue,
...
1. Never use Sauce Labs Mobile App Distribution Auto-update with apps that are shipped to production. This is a clear violation of both Apple and Google's terms.

## Disabling Sauce Labs Mobile App Distribution in Production

When it comes to using Sauce Labs Mobile App Distribution in a production environment, there may be instances where you need to disable Sauce Labs Mobile App Distribution's functionality temporarily or permanently. Disabling Sauce Labs Mobile App Distribution in production can be necessary for a variety of reasons, such as complying with platform guidelines or eliminating potential performance impacts.. The SDK is very modular and is built to handle the your and your company's needs.
...
Navigate to project build settings and locate **Excluded Source File Name** option. Expand the list and find the build scheme you want to exclude Sauce Labs Mobile App Distribution from. Double click the row add two entries to the excluded file list, one for **TestFairy.h**, one for **libTestFairy.a** files.

Try building your project. If the compilation fails, locate the lines where Sauce Labs Mobile App Distribution is used and wrap them with `#ifdef` or `#if` directives explained in Option 1.
...
In order for ProGuard to fully crop the Sauce Labs Mobile App Distribution SDK from the final binary, you may use a wrapper class that differs in each of your flavors.
...
Without any calls to any of the Sauce Labs Mobile App Distribution SDK, Proguard removes the entire compiled code from the result `classes.dex` and the final APK.

#### Option 2: Use a Class Loader

Android allows advanced developers to load classes into memory on-the-fly. You can use Java reflections to load Sauce Labs Mobile App Distribution class into memory only on a Debug build.
