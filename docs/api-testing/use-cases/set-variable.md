...
To make it dynamic and let the engine evaluate the value every time the test will be executed, you have to write it in ${...} brackets.
...
For example, you have a JSON Web Token (JWT) token stored in the `token` variable and we need to decode it and return the JSON payload it was generated from:
...
To execute this, you need to write the following script inside the `Body` field:
