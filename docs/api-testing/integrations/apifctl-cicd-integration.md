...
```bash
docker run --pull quay.io/saucelabs/apifctl run
```
...
```bash
docker run --pull quay.io/saucelabs/apifctl run-all
```
...
```bash title="Full Example"
docker run --pull quay.io/saucelabs/apifctl run-all -H \
...
```bash
docker run --pull quay.io/saucelabs/apifctl run-tag
```
...
```bash
docker run --pull quay.io/saucelabs/apifctl exec
```
...
```bash title="Full Example"
docker run --pull -v "$(pwd)/tests:/tests" quay.io/saucelabs/apifctl exec \
-H https://{SAUCE_USERNAME}:{SAUCE_ACCESS_KEY}@api.us-west-1.saucelabs.com/api-testing/rest/v4/{hookId} \
-p /tests/test_abc \
-n local_test \
...
```bash
docker run --pull quay.io/saucelabs/apifctl upload
```
...
```bash title="Full Example"
docker run --pull -v "$(pwd)/tests:/tests" quay.io/saucelabs/apifctl upload \
-H https://{SAUCE_USERNAME}:{SAUCE_ACCESS_KEY}@api.us-west-1.saucelabs.com/api-testing/rest/v4/{hookId} \
-p /tests/test_abc \
-n local_test \
-t product
...
```bash
docker run --pull quay.io/saucelabs/apifctl vault-get
```
...
```bash
docker run --pull quay.io/saucelabs/apifctl vault-update
```
...
```bash
docker run --pull quay.io/saucelabs/apifctl events  
```
...
```bash
docker run --pull quay.io/saucelabs/apifctl event
```
...
```bash
docker run --pull quay.io/saucelabs/apifctl metrics
```
