...
This feature allows you to create a test starting with a specification file. From the interstitial page, choose the **Build from SPEC** icon.
...
If you choose to upload your spec file, start by clicking the **Attach File** button, then select the correct file from the popup.
...
This feature allows you to create multiple tests from one specification file. From within the project page (where you would like the tests to be stored), choose the **New Tests from SPEC** icon at the top.

<img src={useBaseUrl('img/api-fortress/2020/02/Screen-Shot-2020-02-06-at-2.58.02-PM.png')} alt="Mass Tests from a spec"/>

> **Note**: This process will create one test per path chosen in the specification file inside of the project you are in.
...
Choose as many as you wish to test or select one then click **Select All** in the top left to select all paths and then click **Import selected**. The tests will then be created and named in the following syntax `"method endpoint - status\_code"` (i.e. `"GET /v1/users/ - 200"`).
