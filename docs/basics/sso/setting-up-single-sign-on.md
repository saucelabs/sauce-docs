...
We recommend setting it to the email address, as this makes it easier to manage through the IdP. When the SSO user name is created, only the section preceding the @ is used. For example, from the email **{SAUCE_USERNAME}<span></span>@<span></span>yourcompany.com**, only **{SAUCE_USERNAME}** would be used.

If the **saml:NameID** field contains only a user name and no @ symbol, then the SSO username will be based on your Domain Name. If your domain was sauce and the field value was **{SAUCE_USERNAME}**, the SSO user name would be **sso-sauce-{SAUCE_USERNAME}**.
