...
* **Appium Capabilities:** Required for any test using Appium, either testing web browsers or apps
  ...
  * **Mobile Web Capabilities:** If you are using Appium to test a web app, you need to set the `deviceName`, `platformName` `app`, and `automationName` the same way you would for a mobile app test, along with settings for the browser such as `browserName` and `browserVersion`.
