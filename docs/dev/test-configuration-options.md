...
Depending on which environment you are running tests in, different options, also known as **Capabilities** (or options classes), should be set. There are different sets of capabilities for different environments, which can be combined. These configurations are added to the Capabilities or Options classes. Some of these setting are required for a test to run in a given environment, while some are optional. Certain capabilities
