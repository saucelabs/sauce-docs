...
| **Disabled** | Listed, not installable | Distribution is paused by an admin. Testers still see the build in the list with a **Disabled** badge, but the install buttons are hidden. |
| **Certificate expired** | Listed, not installable | The build's iOS provisioning profile has expired. Testers still see the build in the list with a **Certificate expired** badge, but the install buttons are hidden. Read from the profile at upload — not a manual toggle. |

:::note
Deleting a build removes it immediately; it no longer appears in any state.
:::
...
<svg viewBox="0 0 880 480" width="100%" style={{maxWidth: '880px', height: 'auto'}} role="img" aria-label="Build state diagram: upload leads to Enabled; Enabled and Disabled toggle via disable and enable and together form the live stage; Enabled and Disabled move to Certificate expired when the profile date passes.">
  <defs>
...
:::note
An iOS build uploaded with an *already-expired* profile starts in **Certificate expired**. The app-level **Disable distribution** checkbox overrides everything above - while it is on, no build in the app is installable regardless of its state.
:::
...
**Step 1:** Open the project, locate the build you want to manage, and click the **⋯** (more actions) icon on the build's row.
...
To pause distribution for **all** builds of an app at once, open the app, click **Edit**, and check **Disable distribution** on the **Edit App** page. Click **Save**.
...
## Deleting Several Builds

On the **Builds** tab, select the checkboxes of the builds you want to remove, then click **Delete**.
...
- The state clears when you upload a build signed with a current profile. If you upload the same version number and duplicate versions are off, the existing build is updated with the new file and expiry date.
