...
While the model may receive raw data, we avoid sending personally identifiable information (PII) whenver possible. The only user-related data sent to the LLM are user IDs and usernames.
...
Data processing is performed with in secure and controlled cloud environments utilizing a powerful, compliant large language model.
