...
1. For the **Auth type** field, select the authentication method. Options are:
   ...
   - **OAuth 1.0**: You can either generate the RSA key pair or use the RSA key pair generated by Backtrace. If you want to use the RSA key pair generated by Backtrace, copy and save the public key.
   - **OAuth 2.0**: Enter the **Client ID** and **Client Secret**.
1. Click **Continue**.
...
1. Click **Allow**.
1. In Backtrace, go back to the setup for the Jira Server integration, then click **Continue** to configure the integration.
...
1. Under **Permission**, select **Write**.
1. Click **Save**.
   Jira Server will return the following:
   - **Client ID**
   - **Client Secret**
1. Copy the **Client ID** and **Client Secret** to complete the setup for the Jira Server integration in Backtrace.
