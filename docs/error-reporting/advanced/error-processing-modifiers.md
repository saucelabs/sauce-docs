...
- `_mod_faulting_tid`: This attribute can be set on the object submission of minidump objects. If set, it specifies the thread to use for deduplication. The value is the integer identifier of a thread. This is typically used to incorporate watchdog systems for detecting hung threads.
