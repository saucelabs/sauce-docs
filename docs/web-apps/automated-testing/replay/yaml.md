...
`saucectl` supports using [Sauce Connect](/secure-connections/sauce-connect-4/proxy-tunnels/) to establish a secure connection with Sauce Labs. To do so, launch a tunnel, and then provide the name and owner (if applicable) in this property.
...
:::caution
[Only certain HTTP(S) ports](/secure-connections/sauce-connect-4/advanced/specifications/#supported-browsers-and-ports) are proxied by the tunnel.
:::
