...
When sharding is configured, and used in conjunction with `@cypress/grep`, it may happen that some spec files are allocated to VMs, just to be skipped by playwright in accordance with the `@cypress/grep` filters, thus wasting VM allocations.

With `shardGrepEnabled: true`, saucectl will ensure that specs contains at least one test matching `@cypress/grep` filters.
