...
Available browser names: `chrome`, `firefox`, `microsoftedge`, `webkit`.

:::note WebKit Support
To use WebKit, you must enable the `experimentalWebKitSupport` flag in your Cypress configuration file. See
the [WebKit limitations](/web-apps/automated-testing/cypress/limitations#webkit) for configuration details and known issues.
:::
