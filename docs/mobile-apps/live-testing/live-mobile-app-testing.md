...
| Device Passcode <br/><p><span className="sauceDBlue">ANDROID ONLY</span></p> | Enable/disable setting up device passcode on the device. If you are testing an Android Mobile app which requires a Passcode to be set on the device, this setting enables you to setup the passcode so your application can be launched. |
