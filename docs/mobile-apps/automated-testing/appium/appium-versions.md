...
- We offer a `stable` image, which is our LTS (Long-time support) image. We change this image as conservatively as possible. The regular update schedule is 1 year. We will inform you when this image will be updated and offer a migration guide for all breaking changes from one `stable` image to a new `stable` image. The stable image is subject to change, to ensure the latest OS versions and device models are continuously supported with the latest Appium drivers. If we release a new `stable` image, we will release a preview, suchas `stable-2025`. You will be able to adjust to this new stable image, before we promote this image to the new `stable` image.

- Additionally, we offer a `latest` version. The regular update schedule of this image is quaterly, but we might update this image on demand, if an device-OS update introduces a widespread breaking change, that can only be resolved by updating appium drivers. This version includes all the most recent Appium 2 drivers and server at the time of the release. This image does not have an EOL, since its updated regularly. Use this image if you want to leverage the most recent appium bug fixes and features in your tests, but beware that your tests might break, if you use depreacted appium functionality.
