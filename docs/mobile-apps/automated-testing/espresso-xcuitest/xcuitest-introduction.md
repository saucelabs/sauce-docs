...
`saucectl` supports running tests in parallel. To tests in parallel, configure `saucectl` as follows. See [yaml configuration](./xcuitest.md#shard) for more details.
...
2. Click the `Build Phases`
3. In the `Build Phases`, click the `+`-sign
...
5. The script can be triggered with
    ...
    * Or through running this command
```
xcodebuild \
  CODE_SIGN_IDENTITY="" \
